...
**As of 2026-08-16, this project moved from a Mac to a Windows machine.** The Mac-specific notes below (`/opt/devkitpro`, `mGBA.app`, `./reload`) are stale history, kept for reference — the current environment is Windows 10 + WSL2 (Ubuntu), described first.

### Current: Windows + WSL2 (Ubuntu)
...
### Historical: Mac (superseded 2026-08-16)
...
- A working toolchain **was** installed at `/opt/devkitpro/devkitARM/bin` (includes newlib). `./reload` (repo root) was the edit-build-play loop: it exports `DEVKITARM`/`DEVKITPRO` correctly, discards Porymap no-op rewrite noise from `git status`, runs a full `make -j`, then quits/relaunches `mGBA.app` with the freshly built `pokeemerald.gba`.
