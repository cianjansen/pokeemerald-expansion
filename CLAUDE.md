...
**If the trainer needs to say/do more right after winning (like a Gym Leader) — use `trainerbattle_single`'s 4th arg, don't rely on falling through.** Without an `event_script` argument, `trainerbattle_single trainer, IntroText, DefeatText` unlocks the player right after `DefeatText` on a win — anything written after that command in the same script only runs on a *later, separate* re-talk (because on that re-talk the trainer's already defeated, so `trainerbattle_single` no-ops and falls through immediately). If you want extra dialog/`givemon`/`giveitem`/`setflag` to run automatically, still locked, in the same breath as the win — e.g. handing over a reward — pass a 5th-arg-style event script: `trainerbattle_single TRAINER_X, IntroText, DefeatText, EventScript_PostBattle`. That script runs exactly once, automatically, the moment the trainer is first defeated (see `RustboroCity_Gym_EventScript_Roxanne` for the vanilla reference, or `Ouderkerkerplas_EventScript_Jonas`/`_JonasPostBattle` for this hack's own copy of the pattern). Got this wrong once already for Jonas's bike hand-off — it silently required a second, separate talk to trigger instead of running right after the win.
